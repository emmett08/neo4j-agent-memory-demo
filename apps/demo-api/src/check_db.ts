...
async function main() {
  ...
  try {
    ...
    console.log("\n=== Labels in database ===");
    labels.records.forEach(r => console.log("  -", r.get("label")));
    
    // Check relationship types
    const rels = await session.run("CALL db.relationshipTypes() YIELD relationshipType RETURN relationshipType");
    console.log("\n=== Relationship types ===");
    rels.records.forEach(r => console.log("  -", r.get("relationshipType")));
    ...
    casesWithMemories.records.forEach(r => {
      console.log(`\n  Case: ${r.get("caseTitle")}`);
      const fixes = r.get("fixes").filter((f: any) => f);
      const negatives = r.get("negatives").filter((n: any) => n);
      if (fixes.length > 0) {
